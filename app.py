...
def predict_emotion11(sample_text, model):
    ...
    print("Prediction: {}, Prediction Score:{}" .format(res[0],np.max(pred_proba)))
    keys = pred_percentage_for_all.keys()
    values = pred_percentage_for_all.values()
    ...
    html = (
        '<link href="https://fonts.googleapis.com/css2?family=Playball&display=swap" rel="stylesheet"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Merriweather:wght@700&family=Source+Serif+Pro:ital@1&display=swap" rel="stylesheet"><link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css"><style>header{font-family:Playball, cursive;color: #843f5b;font-size: 70px;margin-top: 40px;text-align: center;}.f{font-family: Merriweather, serif;font-family: Source Serif Pro, serif;font-size:30px;text-align: center;color:#A95C68}body{font-family: Merriweather, serif;font-family: Source Serif Pro, serif;font-size:20px;}.btn {background-color:#702963;border: none; color: white;padding: 5px; text-align: center; text-decoration: none;display: inline-block;font-size: 25px;width: 120px;margin-left: 50%;cursor: pointer;}.grid {display: grid;grid-template-columns: repeat(5, 1fr); grid-gap: 5px; margin-left: 20%;} </style><header><div><b>Know your Insights..</b></div></header>'
        + "<img src='data:image/png;base64,{}'>".format(encoded)
        + '<br><div class="f"><b>Overall Emotion: {{result[0]}}</b></div><br><br><div class="grid"><button type="button" class="btn" id="back">Back</button><br><br><button type="button" class="btn" id="continue">Continue</button></div><br><br><script type="text/javascript">document.getElementById("back").onclick = function () { location.href = "/quiz1";};</script> <script type="text/javascript">document.getElementById("continue").onclick = function () { location.href = "/login";};</script>'
    )
    with open("D:/MentalHealthProject/templates/test.html", "w") as f:
        f.write(html)
    return res
...
def result():
    ...
    return render_template("test.html", result=prediction)
